        ...
        Object.entries(elementAttributes)
          ...
          .map(([key, value]) => (
            <div key={`input-group-${key}`} className="input-group mb-2">
              ...
              <input type="text" className="form-control" name="value-form" value={value} readOnly={key === 'id'} onChange={handleFormChange(key)} />
              {key !== 'id' &&
                <div className="input-group-append ml-1">
                  <button type="button" className="close" aria-label="Close" onClick={onDeleteAttributeClick(key)}>
